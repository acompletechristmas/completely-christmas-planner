...
3. **Assemble a balanced menu** — take the top N per course with per-course caps that depend on group size (couple: 1 starter, 1 main, 2–3 sides, 1 dessert; large group: more sides, more shareables; buffet style skips starter/main and uses Cold/Hot/Cheese). Guarantee at least one suitable option per active dietary tag (a vegan guest forces a vegan main and a vegan side into the menu).
