...
- Reuse existing imagery only where it genuinely matches the quality, composition and Christmas style shown in the reference image. Where an existing image does not achieve the reference look, create a more suitable premium realistic Christmas image. The reference image is the visual target. Do not keep a weaker existing image simply to avoid creating a new one.
